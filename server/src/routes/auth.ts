...
authRouter.post('/register', async (req: Request, res: Response) => {
  ...
  try {
    await withConn(async (conn) => {
      ...
      const [result] = await conn.query('INSERT INTO users (email, password_hash, tz, recent_logs_scope, search_default_range) VALUES (?, ?, ?, ?, ?)', [email, pwdHash, tz || 'UTC', 'wtd_prev', 'wtd_prev'])
...
authRouter.post('/login', async (req: Request, res: Response) => {
  ...
  try {
    await withConn(async (conn) => {
      const [rows] = await conn.query('SELECT id, password_hash, tz, role, recent_logs_scope, search_default_range FROM users WHERE email = ? LIMIT 1', [email])
      ...
      res.cookie('rt', plain, cookieOptions(req))
      res.json({ user: { id: user.id, email, tz: user.tz, role: user.role, recent_logs_scope: user.recent_logs_scope || 'wtd_prev', search_default_range: user.search_default_range || 'wtd_prev' } })
    })
  } catch (err) {
  ...
  try {
    await withConn(async (conn) => {
      ...
      const [rows] = await conn.query(
        `SELECT s.id, s.user_id, u.email, u.tz, u.role, u.recent_logs_scope, u.search_default_range FROM sessions s
      ...
      res.cookie('rt', plain, cookieOptions(req))
      res.json({ user: { id: sess.user_id, email: sess.email, tz: sess.tz, role: sess.role, recent_logs_scope: (sess as any).recent_logs_scope || 'wtd_prev', search_default_range: (sess as any).search_default_range || 'wtd_prev' } })
    })
  } catch (err) {
...
authRouter.get('/me', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await withConn(async (conn) => {
      const [rows] = await conn.query(
        `SELECT u.id, u.email, u.tz, u.role, u.recent_logs_scope, u.search_default_range FROM sessions s
         JOIN users u ON u.id = s.user_id
         WHERE s.token_hash = ? AND s.revoked_at IS NULL AND s.expires_at > NOW(6)
         LIMIT 1`,
        [tokenHash]
      ...
      const u = arr[0]
      res.json({ user: { id: u.id, email: u.email, tz: u.tz, role: u.role, recent_logs_scope: u.recent_logs_scope || 'wtd_prev', search_default_range: u.search_default_range || 'wtd_prev' } })
    })
  } catch (err) {
...
authRouter.patch('/me', requireAuth as any, async (req: Request, res: Response) => {
  const { tz, recent_logs_scope, search_default_range } = req.body || {}
  if (!tz && !recent_logs_scope && !search_default_range) return res.status(400).json({ error: 'tz or recent_logs_scope or search_default_range required' })
  const allowedRecent = new Set(['wtd','wtd_prev','mtd','mtd_prev'])
  const allowedSearch = new Set(['wtd','wtd_prev','prev_week','all_weeks','mtd','mtd_prev','prev_month','all_months','all_records'])
  try {
    await withConn(async (conn) => {
      const token = sha256Hex((req as any).cookies?.rt as string)
      if (tz && recent_logs_scope && allowedRecent.has(String(recent_logs_scope))) {
      ...
      } else if (recent_logs_scope && allowedRecent.has(String(recent_logs_scope))) {
      ...
      if (search_default_range && allowedSearch.has(String(search_default_range))) {
        await conn.query('UPDATE users SET search_default_range = ? WHERE id = (SELECT user_id FROM sessions WHERE token_hash = ? LIMIT 1)', [String(search_default_range), token])
      }
      res.json({ ok: true })
    })
  } catch (err) {
